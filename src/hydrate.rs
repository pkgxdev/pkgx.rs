...
pub async fn hydrate<F>(input: Vec<PackageReq>, get_deps: F) -> Result<Vec<PackageReq>, Box<dyn Error>>
where
    F: Fn(String) -> Result<Vec<PackageReq>, Box<dyn Error>>,
{
    ...
    for pkg in dry.iter() {
        ...
        node.pkg.constraint = intersect_constraints(
            &node.pkg.constraint,
            &pkg.constraint,
        )?;
    ...
    while let Some(mut current) = stack.pop() {
        for child_pkg in get_deps(current.pkg.project.clone())? {
            ...
            child_node.pkg.constraint = intersect_constraints(
...
fn condense(pkgs: Vec<PackageReq>) -> Vec<PackageReq> {
    ...
    for pkg in pkgs {
        if let Some(existing) = out.iter_mut().find(|p| p.project == pkg.project) {
            existing.constraint = intersect_constraints(&existing.constraint, &pkg.constraint).unwrap();
        } else {
            out.push(pkg);
        }
    }
